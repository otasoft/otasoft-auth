...
export class SetRefreshTokenHandler implements ICommandHandler<SetRefreshTokenCommand>{
    constructor(
    ...
    async execute(command: SetRefreshTokenCommand) {
        const hashedRefreshToken = await this.passwordUtilsService.hashContent(command.refreshToken, 10);
